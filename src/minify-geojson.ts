        ...
        if (fs.existsSync(file)) {
            loadFile(file, options, (geojson) => {
                ...
                if (options.from) {
                    ...
                    getCoordinateReferenceSystem(crsName, (crss) => {
                        let reproject = require('reproject');
                        geojson = reproject.toWgs84(geojson, crss, crss);                    
                        processGeoJSON(geojson, file, outputFile, options, () => {});
                    });
                } else {
                    processGeoJSON(geojson, file, outputFile, options, () => {});
                }
 ...
 */
function loadFile(inputFile: string, options: ICommandOptions, cb: (geojson?: GeoJSON.FeatureCollection<GeoJSON.GeometryObject>) => void) {
 ...
 * 
 */
function getCoordinateReferenceSystem(crsName: string, cb: (crss: Object) => void) {
