...
export const createPropertyFilter = (prop: string, op: FilterOperatorType, val: string | number) => {
  switch (op) {
    ...
    case '=':
      return (props: Object) => props.hasOwnProperty(prop) && props[prop] == val;
    case '<':
      return (props: Object) => props.hasOwnProperty(prop) && props[prop] < val;
    case '>':
      return (props: Object) => props.hasOwnProperty(prop) && props[prop] > val;
...
export const convertQueryToPropertyFilters = (filterQuery: string): Array<(props: Object) => boolean> => {
  ...
  queries.forEach((q) => {
    ...
    if (!m || m.length !== 4) {
      const errMsg =
        'Filters should be in the form PROPERTY OPERATOR VALUE, where property is a string, OPERATOR is <, =, >, !=, <= or >=, and VALUE is a string or number.';
